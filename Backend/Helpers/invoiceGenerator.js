...
export const generateInvoicePDF = async (invoice) => {
  // 1. Define and Create the directory path if it doesn't exist
  const dir = path.join(process.cwd(), "uploads", "invoices");
  if (!fs.existsSync(dir)) {
    fs.mkdirSync(dir, { recursive: true });
  }

  // 2. Define the unique filename (replacing slashes to avoid path errors)
...
`;
  ...
  // 3. Save to disk AND return buffer
  const pdfBuffer = await page.pdf({
    path: filePath, // ✅ This saves the file to uploads/invoices/
    format: "A4",
  ...
  // 4. We return both or just the buffer depending on your controller needs
  return pdfBuffer;
};