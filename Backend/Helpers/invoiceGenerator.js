...
export const generateInvoicePDF = async (invoice) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fontSize(8.5).fillColor(textDark).font("Helvetica-Bold").text(`GSTIN: ${invoice.sellerGstin || "YOUR_GSTIN_HERE"}`, ML, 82);
      ...
      doc.fontSize(8.5).fillColor(textMid).font("Helvetica")
        ...
        .text(`Date: ${invoice.date}`, ML, 89, { align: "right", width: CW });

      doc.moveTo(ML, 105).lineTo(ML + CW, 105).strokeColor(gold).lineWidth(1.5).stroke();

      // --- Address & Payment Blocks (Fixed Overlap) ---
      const infoY = 120;
      const colW = CW / 2;

      // BILL TO
      doc.fontSize(10).fillColor(gold).font("Helvetica-Bold").text("BILL TO", ML, infoY);
      doc.fontSize(9).fillColor(textDark).font("Helvetica-Bold").text(invoice.buyerName, ML, infoY + 18);
      ...
      const paymentLines = [
        ...
        { l: "Status", v: "SUCCESS" },
        { l: "Txn ID", v: invoice.transactionId || "—" }
      ];
      ...
      invoice.items.forEach((item, i) => {
        ...
        doc.text(`₹${item.unitPrice.toFixed(2)}`, ML + cols[2].x, rowY + 6, { width: cols[2].w - 5, align: "right" });
        doc.text(`₹${item.taxableValue.toFixed(2)}`, ML + cols[3].x, rowY + 6, { width: cols[3].w - 5, align: "right" });
        doc.text(`₹${(item.cgst + item.sgst).toFixed(2)}`, ML + cols[4].x, rowY + 6, { width: cols[4].w - 5, align: "right" });
        doc.text(`₹${item.finalPrice.toFixed(2)}`, ML + cols[5].x, rowY + 6, { width: cols[5].w - 5, align: "right" });
        rowY += 20;
      });

      // --- Invoice Summary (Separate CGST/SGST) ---
      const sumY = rowY + 20;
      const sumW = 230;
      ...
      const drawRow = (label, value, color = textDark) => {
        ...
        doc.fillColor(color).text(`₹${value.toFixed(2)}`, sumX, subY, { width: sumW - 10, align: "right" });
        subY += 16;
      };

      drawRow("Item Total (Incl. GST)", invoice.subtotal);
      drawRow("Less: Discount", Math.abs(invoice.discount), "#CC3300");
      ...
      doc.text(`₹${invoice.totalPaid.toFixed(2)}`, sumX, sumY + 113, { width: sumW - 10, align: "right" });
